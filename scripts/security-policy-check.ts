...
import { analyzePnpmVersionParity, analyzeWorkflowText, normalizeInlineYamlScalar, PNPM_SETUP_PARSER_SCOPE, WORKFLOW_POLICY_PARSER_SCOPE, WORKFLOW_YAML_POLICY_SCOPE } from "./workflow-policy";
...
function main(): void {
  ...
  console.log(`WORKFLOW_YAML_POLICY_SCOPE=${WORKFLOW_YAML_POLICY_SCOPE}`);
  console.log(`PNPM_SETUP_PARSER_SCOPE=${PNPM_SETUP_PARSER_SCOPE}`);
  ...
  const pnpmVersionFindings: string[] = [];
  const packageManager = (JSON.parse(fs.readFileSync("package.json", "utf8")) as { packageManager?: unknown }).packageManager;
  const packageManagerPin = typeof packageManager === "string" ? packageManager : "";

  for (const file of workflows) {
    const relative = path.relative(process.cwd(), file);
    const content = fs.readFileSync(file, "utf8");
    advancedFindings.push(...analyzeWorkflowText(content).map(finding => `${relative}: ${finding.rule} (${finding.detail})`));
    pnpmVersionFindings.push(...analyzePnpmVersionParity(content, packageManagerPin).map(finding => `${relative}: ${finding.rule} (${finding.detail})`));
    for (const match of content.matchAll(/^\s*(?:-\s*)?(?:uses|["']uses["'])\s*:\s*([^\s#]+)(?:\s*#.*)?$/gm)) {
      const reference = normalizeInlineYamlScalar(match[1]);
  ...
  const checks: Check[] = [
    ...
    { name: "pnpm workflow version parity", passed: pnpmVersionFindings.length === 0, details: pnpmVersionFindings.length ? pnpmVersionFindings.join(", ") : `Workflow pnpm setup matches ${packageManagerPin}.` },
